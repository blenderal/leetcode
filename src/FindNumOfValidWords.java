    ...
    public static List<Integer> findNumOfValidWords(String[] words, String[] puzzles) {
        ...
        for (String word : words) {
            ...
            for (int j = 0; j < size; j++) {
                bitCount |= (1 << (word.charAt(j) - 'a'));
            }
            if (Integer.bitCount(bitCount) <= 7) {
        ...
        for (String puzzle : puzzles) {
            ...
            int bitCount = 0;
            for (int i = 1; i < 7; i++) {
                bitCount |= 1 << (puzzle.charAt(i) - 'a');
            }
            int subBit = bitCount;
            do {
                count += map.getOrDefault(subBit | (1 << (puzzle.charAt(0) - 'a')), 0);
                subBit = (subBit - 1) & bitCount;
            } while (bitCount != subBit);
            ret.add(count);
        }
        return ret;
    }
